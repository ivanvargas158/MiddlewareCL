    ...
    try:        
        ...
        if len(list_attachments)==len(list_documents):
            ...
            for attachment in list_attachments:
                ...
                if not attachmentId :
                    raise Exception(f"Invalid attachmentId")        
        ...
        raise Exception(f"Request error ocr_process_document: {e.args}")

    except Exception as e:
        raise Exception(f"Unexpected error ocr_process_document: {e.args}")
...
def  call_ocr(file_name:str,mime_type:str,file:io.BytesIO,country_id:int)-> Dict:
    try: 
        ...
        response = requests.post(url, headers=headers, files=new_file)         
